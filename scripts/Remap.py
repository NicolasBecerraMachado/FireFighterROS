...
def mapping(val, in_min, in_max, out_min, out_max):
        ...
        return res if (255 >= res) else 255
...
class Remap():
    def __init__(self): 
        ...
        self.min_pwm = [0, 0, 0, 0]
        self.min_speed = [0, 0, 0, 0] #please update
        self.max_speed = [180.8*2*math.pi/60, 187.0*2*math.pi/60, 178.2*2*math.pi/60, 179.046*2*math.pi/60]
        ...
        self.pwm0 = rospy.Publisher('arduino/pwm1', Int32, queue_size=10)
        self.pwm1 = rospy.Publisher('arduino/pwm2', Int32, queue_size=10)
        self.pwm2 = rospy.Publisher('arduino/pwm3', Int32, queue_size=10)
        self.pwm3 = rospy.Publisher('arduino/pwm4', Int32, queue_size=10)

        self.pwm = [Int32(), Int32(), Int32(), Int32()]
    ...
    def update_pwm0(self,w):
        ...
        if back:
            act_pwm -= act_pwm
        """
        act_pwm = mapping(w,self.min_speed[0],self.max_speed[0],self.min_pwm[0],255)
        if back:
            act_pwm = -act_pwm
    ...
    def update_pwm1(self,w):
        ...
        if back:
            act_pwm -= act_pwm
        """
        act_pwm = mapping(w,self.min_speed[1],self.max_speed[1],self.min_pwm[1],255)
        if back:
            act_pwm = -act_pwm
    ...
    def update_pwm2(self,w):
        ...
        if back:
            act_pwm -= act_pwm
        """
        act_pwm = mapping(w,self.min_speed[2],self.max_speed[2],self.min_pwm[2],255)
        if back:
            act_pwm = -act_pwm
    ...
    def update_pwm3(self,w):
        ...
        if back:
            act_pwm -= act_pwm
        """
        act_pwm = mapping(w,self.min_speed[3],self.max_speed[3],self.min_pwm[3],255)
        if back:
            act_pwm = -act_pwm
